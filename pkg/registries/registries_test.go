...
func TestRegMatchesScope(t *testing.T) {
	for _, tt := range []struct {
		...
		{"bar/example.foo", "*.foo", false},                 // Invalid wildcard entry
		{"example/bar.foo/quay.io", "*.foo", false},         // Invalid wildcard entry
		{"example/bar.foo:400", "*.foo", false},             // Invalid wildcard entry
		{"foo.example.com", "*.example.com", true},          // Valid sub-scope match
		{"*.foo.example.com", "*.example.com", true},        // Valid sub-scope match
		{"foo.example.com/bar", "*.example.com", true},      // Valid sub-scope match
		{"foo.registry.com", "*.example.com", false},        // Invalid sub-scope match
		{"foo.example.com", "**.example.com", false},        // Invalid sub-scope entry
		{"foo.example.com", "example.*.com", false},         // Invalid sub-scope entry
		{"foo.example.com", "*.example.com/foo/bar", false}, // Invalid wildcard entry
		...
		t.Run(fmt.Sprintf("%#v, %#v", tt.scope, tt.reg), func(t *testing.T) {
			res := regMatchesScope(tt.scope, tt.reg)
			assert.Equal(t, tt.expected, res)
		})
	}
}

func TestIsValidRegistryOrWildcardScope(t *testing.T) {
	for _, tt := range []struct {
		scope    string
		expected bool
	}{
		...
		t.Run(fmt.Sprintf("%#v", tt.scope), func(t *testing.T) {
			res := IsValidRegistryOrWildcardScope(tt.scope)
			assert.Equal(t, tt.expected, res)
		})
	}
}
